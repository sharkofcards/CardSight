# Create a refactored Streamlit app with SQLite caching, rate limiting, and background refresh.
# The file will be saved to /mnt/data/CardSight_refactor.py so the user can download or run it.
...
# --------------- CONFIG ---------------
# Tune these safely within your eBay rate limits.
CALLS_PER_SECOND = 4           # <- conservative; change to your real allowed rate
CALLS_PER_DAY_BUDGET = 8000    # <- total daily budget you want to stay under
CACHE_STALE_HOURS = 6          # cache freshness window
MAX_PAGES = 5                  # max pages to fetch per query refresh
PAGE_SIZE = 100                # items per page if supported
BACKGROUND_REFRESH = True      # spawn a background refresh so UI returns cached results fast

# eBay API config (replace with your keys/endpoint if needed)
